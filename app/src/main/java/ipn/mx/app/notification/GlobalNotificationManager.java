...
public class GlobalNotificationManager {
    ...
    NotificationManagerDaily notificacionManagerDaily;
    Intent notifyIntent;
    ...
    public GlobalNotificationManager(Context context, NotificationManagerDaily notificacionManagerDaily) {
        this.context = context;
        notificationManagerCompat = NotificationManagerCompat.from(context.getApplicationContext());
        this.notificacionManagerDaily = notificacionManagerDaily;
    }
    ...
    public void generateNotificationDaily() {
        Log.d(TAG, "generateNotificationDaily()");

        // 1. Create/Retrieve Notification Channel for O and beyond devices (26+).
        String notificationChannelId =
                NotificationUtil.createNotificationChannel(context, notificacionManagerDaily);


        // 2. Build the BIG_TEXT_STYLE.
        NotificationCompat.BigTextStyle bigTextStyle = new NotificationCompat.BigTextStyle()
                // Overrides ContentText in the big form of the template.
                .bigText(notificacionManagerDaily.getBigText())
                // Overrides ContentTitle in the big form of the template.
                .setBigContentTitle(notificacionManagerDaily.getBigContentTitle())
                // Summary line after the detail section in the big form of the template.
                // Note: To improve readability, don't overload the user with info. If Summary Text
                // doesn't add critical information, you should skip it.
                .setSummaryText(notificacionManagerDaily.getSummaryText());


        // 3. Set up main Intent for notification.
        notifyIntent = new Intent(context, Daily.class);
        ...
        // Strategy Action
        dailyIntent = new Intent(context, Daily.class);

        PendingIntent strategyPendingIntent = PendingIntent.getService(context, 0, dailyIntent, 0);
        NotificationCompat.Action strategyAction =
                new NotificationCompat.Action.Builder(
                        R.drawable.ic_reply_white_18dp,
                        notificacionManagerDaily.getButtonText(),
                        strategyPendingIntent)
                        .build();
        ...
        @SuppressLint("WrongConstant") Notification notification = notificationCompatBuilder
                ...
                .setStyle(bigTextStyle)
                // Title for API <16 (4.0 and below) devices.
                .setContentTitle(notificacionManagerDaily.getContentTitle())
                // Content for API <24 (7.0 and below) devices.
                .setContentText(notificacionManagerDaily.getContentText())
                .setSmallIcon(R.drawable.logo)
                .setLargeIcon(BitmapFactory.decodeResource(
                        context.getResources(),
                        R.drawable.logo))
                ...
                // 'importance' which is set in the NotificationChannel. The integers representing
                // 'priority' are different from 'importance', so make sure you don't mix them.
                .setPriority(notificacionManagerDaily.getPriority())

                // Sets lock-screen visibility for 25 and below. For 26 and above, lock screen
                // visibility is set in the NotificationChannel.
                .setVisibility(notificacionManagerDaily.getChannelLockscreenVisibility())
                .addAction(strategyAction)
